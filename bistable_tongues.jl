...
for i in 1:max_i_ratio
    for j in 1:max_j_ratio
        tongues[:, :, i, j] = readdlm(string("tongues_zoom_1/tongue_", i, j, ".csv"))
    end
end
...
for (i, per) in ProgressBar(enumerate(pers))
    ...
    for i in 1:max_i_ratio
        for j in 1:max_j_ratio
            writedlm(string("tongues_zoom_1/tongue_", i, j, ".csv"), tongues[:, :, i, j])
        end
    end
...
for i in 1:max_i_ratio
    for j in 1:max_j_ratio
        ...
        heatmap!(plt, pers, amps, readdlm(string("tongues/tongue_", i, j, ".csv")), color=pal[c_id], alpha=0.8)
        c_id += 1
    end
end
